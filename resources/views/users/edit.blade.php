...
<div class="row">
  <div class="col-md-8 col-md-offset-2">
    ...
    $currentUserPermissionsArray = is_array($currentUserPermissions) ? $currentUserPermissions : json_decode($currentUserPermissions, true);
    $isSuperuser = !empty($currentUserPermissionsArray['superuser']) && $currentUserPermissionsArray['superuser'] == 1;

    $targetUserPermissions = $user->permissions;
    // Ensure the target user's permissions are in array format
    $targetUserPermissionsArray = is_array($targetUserPermissions) ? $targetUserPermissions : json_decode($targetUserPermissions, true);
    $isTargetSuperuser = !empty($targetUserPermissionsArray['superuser']) && $targetUserPermissionsArray['superuser'] == 1;
@endphp
    ...
    <div class="col-md-6{{ Helper::checkIfRequired($user, 'password') ? ' required' : '' }}">
        @if ($user->ldap_import != '1' || str_contains(Route::currentRouteName(), 'clone'))
            @if ($isTargetSuperuser && !$isSuperuser)
                <p class="alert-msg">{{ trans('You cannot change the password of a superuser because you do not have the required rights.') }}</p>
            @else
                <input
                    type="password"
                    name="password"
