            ...
            <form class="form-horizontal" method="post" autocomplete="off"
                                ...
                                <div class="col-md-12">
                                    ...
                                    <div class="form-group {{ $errors->has('first_name') ? 'has-error' : '' }}">
                                        <label class="col-md-3 control-label"
                                            for="first_name">{{ trans('general.first_name') }}</label>
                                        <div
                                            class="col-md-6{{ Helper::checkIfRequired($user, 'first_name') ? ' required' : '' }}">
                                            <input class="form-control" type="text" name="first_name" id="first_name"
                                                value="{{ old('first_name', $user->first_name) }}" maxlength="191" />
                                    ...
                                    <div class="form-group {{ $errors->has('last_name') ? 'has-error' : '' }}">
                                        <label class="col-md-3 control-label"
                                            for="last_name">{{ trans('general.last_name') }} </label>
                                        <div
                                            class="col-md-6{{ Helper::checkIfRequired($user, 'last_name') ? ' required' : '' }}">
                                            <input class="form-control" type="text" name="last_name" id="last_name"
                                                value="{{ old('last_name', $user->last_name) }}" maxlength="191" />
                ...
                @endif --}}
                                    ...
                                    @php
                                        ...
                                        $currentUserPermissionsArray = is_array($currentUserPermissions)
                                            ? $currentUserPermissions
                                            : json_decode($currentUserPermissions, true);
                                        $isSuperuser =
                                            !empty($currentUserPermissionsArray['superuser']) &&
                                            $currentUserPermissionsArray['superuser'] == 1;

                                        $targetUserPermissions = $user->permissions;
                                        // Ensure the target user's permissions are in array format
$targetUserPermissionsArray = is_array($targetUserPermissions)
    ? $targetUserPermissions
    : json_decode($targetUserPermissions, true);
$isTargetSuperuser =
    !empty($targetUserPermissionsArray['superuser']) &&
    $targetUserPermissionsArray['superuser'] == 1;
                                    @endphp
                                    ...
                                    <div class="form-group {{ $errors->has('password') ? 'has-error' : '' }}">
                                        ...
                                        <div
                                            class="col-md-6{{ Helper::checkIfRequired($user, 'password') ? ' required' : '' }}">
                                            @if ($user->ldap_import != '1' || str_contains(Route::currentRouteName(), 'clone'))
                                                @if ($isTargetSuperuser && !$isSuperuser)
                                                    <p class="alert-msg">
                                                        {{ trans('You cannot change the password of a superuser because you do not have the required rights.') }}
                                                    </p>
                                                @else
                                                    <input type="password" name="password" class="form-control"
                                                        id="password" value="" maxlength="500" autocomplete="off"
                                        ...
                                        <div class="col-md-2">
                                            @if ($user->ldap_import != '1' && (!$isTargetSuperuser || $isSuperuser))
                                                <a href="#" class="left"
                                    ...
                                    @if ($user->ldap_import != '1' || str_contains(Route::currentRouteName(), 'clone'))
                                        ...
                                        <div
                                            ...
                                            <div
                                                class="col-md-6{{ Helper::checkIfRequired($user, 'password_confirmation') && !$user->id ? ' required' : '' }}">
                                                @if ($isTargetSuperuser && !$isSuperuser)
                                                    <p class="alert-msg">
                                                        {{ trans('You cannot change the password of a superuser because you do not have the required rights.') }}
                                                    </p>
                                                @else
                                                    <input type="password" name="password_confirmation"
                                                        id="password_confirm" class="form-control" value=""
                                            ...
                                            <div class="col-md-9">
                                                ...
                                                @else
                                                    <div style="padding-left: 10px;">
                                                        <input type="checkbox" value="1" id="activated"
                                                            name="activated" class="minimal"
                                                            {{ old('activated', $user->activated) == '1' ? ' checked="checked"' : '' }}
                                                            aria-label="activated">
                                                        {{ trans('admin/users/general.activated_help_text') }}
                                                    </div>
                                    ...
                                    <div class="form-group {{ $errors->has('email') ? 'has-error' : '' }}">
                                        ...
                                        <div
                                            class="col-md-6{{ Helper::checkIfRequired($user, 'email') ? ' required' : '' }}">
                                            <input class="form-control" type="text" name="email" id="email"
                                                maxlength="191" value="{{ Request::old('email', $user->email) }}"
                                                {{ config('app.lock_passwords') && $user->id ? ' disabled' : '' }}
                                    ...
                                    @if ($user->avatar && $user->avatar != '')
                                        ...
                                        <div class="form-group">
                                            <div class="col-md-9 col-md-offset-3">
                                                <img src="{{ Storage::disk('public')->url(app('users_upload_path') . $user->avatar) }}"
                                        ...
                                        <div id="optional_user_details" class="col-md-12" style="display:none">
                                                        ...
                                                        <div class="col-md-9">
                                                            @if (config('app.lock_passwords'))
                                                                <div class="icheckbox disabled">
                                                                    {{ Form::checkbox('two_factor_optin', '1', Request::old('two_factor_optin', $user->two_factor_optin), ['class' => 'minimal', 'disabled' => 'disabled']) }}
                                                                    {{ trans('admin/settings/general.two_factor_enabled_text') }}
                                                                    <p class="help-block">
                                                                        {{ trans('general.feature_disabled') }}</p>
                                                                </div>
                                                            @else
                                                                {{ Form::checkbox('two_factor_optin', '1', Request::old('two_factor_optin', $user->two_factor_optin), ['class' => 'minimal']) }}
                                                                {{ trans('admin/settings/general.two_factor_enabled_text') }}
                                                                <p class="help-block">
                                            ...
                                            <div class="form-group{{ $errors->has('groups') ? ' has-error' : '' }}">
                                                ...
                                                <div class="col-md-6">
                                                    ...
                                                    @if ($groups->count())
                                                        @if (Config::get('app.lock_passwords') /* || !Auth::user()->isSuperUser() || Auth::user()->permissions['admin'] == '1' */)
                                                        ...
                                                        @endif
                                                    @else
                                                        <p>No groups have been created yet. Visit <code>Admin Settings >
                                                                Permission Groups</code> to add one.</p>
                                                    @endif
                    ...
                    <div class="box-footer text-right">
                        <button type="submit" accesskey="s" class="btn btn-primary"><i class="fas fa-check icon-white"
