                    ...
                    <div class="col-md-12">
                        @if ($backto == 'user')
                            <form class="form-horizontal" method="post"
                                action="{{ route('hardware.checkin.store', ['assetId' => $asset->id, 'backto' => 'user']) }}"
                                autocomplete="off">
                            @else
                                <form class="form-horizontal" method="post"
                                    action="{{ route('hardware.checkin.store', ['assetId' => $asset->id]) }}"
                                    autocomplete="off">
                        @endif
                        {{ csrf_field() }}
                        ...
                        <div class="form-group">
                            {{ Form::label('model', trans('admin/hardware/form.model'), ['class' => 'col-md-3 control-label']) }}
                            <div class="col-md-8">
                        ...
                        <div class="form-group {{ $errors->has('name') ? 'error' : '' }}">
                            {{ Form::label('name', trans('admin/hardware/form.name'), ['class' => 'col-md-3 control-label']) }}
                            <div class="col-md-8">
                        ...
                        <div class="form-group {{ $errors->has('status_id') ? 'error' : '' }}">
                            {{ Form::label('status_id', trans('admin/hardware/form.status'), ['class' => 'col-md-3 control-label']) }}
                            <div class="col-md-7 required">
                                {{ Form::select('status_id', $statusLabel_list, '', ['class' => 'select2', 'style' => 'width:100%', 'id' => 'modal-statuslabel_types', 'aria-label' => 'status_id']) }}
                                {!! $errors->first(
                                    'status_id',
                                    '<span class="alert-msg" aria-hidden="true"><i class="fas fa-times" aria-hidden="true"></i> :message</span>',
                                ) !!}
                        ...
                        @include ('partials.forms.edit.location-select', [
                            ...
                            'help_text' => $asset->defaultLoc
                                ? 'You can choose to check this asset in to a location other than the default location of ' .
                        ...
                        <div class="form-group{{ $errors->has('checkin_at') ? ' has-error' : '' }}">
                            {{ Form::label('checkin_at', trans('admin/hardware/form.checkin_date'), ['class' => 'col-md-3 control-label']) }}
                            <div class="col-md-8">
