...
<div id="{{ $fieldname }}" class="form-group{{ $errors->has($fieldname) ? ' has-error' : '' }}">
    ...
    <div class="col-md-7{{ isset($field_req) || (isset($required) && $required == 'true') ? ' required' : '' }}">
        <select class="js-data-ajax" data-endpoint="models" data-placeholder="{{ trans('general.select_model') }}"
            ...
            aria-label="{{ $fieldname }}"{!! isset($field_req) ? ' data-validation="required" required' : '' !!}>
            ...
            @php
                $model_id = old($fieldname, isset($item) ? $item->{$fieldname} : '');
            @endphp

            @foreach (\App\Models\AssetModel::all() as $assetModel)
                <option value="{{ $assetModel->id }}" {{ $assetModel->id == $model_id ? 'selected="selected"' : '' }}
                    role="option" aria-selected="{{ $assetModel->id == $model_id ? 'true' : 'false' }}">
                    {{ $assetModel->name ?? '' }}
                </option>
            @endforeach
